...
mod init {
    ...
    fn mock_init(
        ...
        let info = mock_info(caller, &coins(amount, DEFAULT_DENOM));
    ...
    pub fn init_default(
        ...
        let msg = InstantiateMsg {
            ...
            denom: Some(DEFAULT_DENOM.to_string()),
            next_swap_id: 0,
        };
    ...
    fn success_init() {
        ...
        // check role
        ac_have_role(&deps.storage, &addr!(DEFAULT_OWNER), &AccessRole::Admin)
...
mod access_control {
    ...
    pub fn grant_role(
        ...
        let info = mock_info(caller, &coins(0, DEFAULT_DENOM));
    ...
    pub fn renounce_role(
        ...
        let info = mock_info(account, &coins(0, DEFAULT_DENOM));
        execute(deps.as_mut(), mock_env(), info, msg)
    }

    fn check_renounce_role_success(deps: DepsMut, response: &Response, role: &str, account: &str) {
        // handle response
        assert_eq!(0, response.messages.len());
        assert_eq!(3, response.attributes.len());
        assert!(
    ...
    fn success_renounce_roles() {
        ...
        check_renounce_role_success(deps.as_mut(), &response, &role, &account);
...
mod pause {
    ...
    fn _pause_public_api(
        ...
        let msg = ExecuteMsg::PausePublicApi { since_block };
        execute(deps.as_mut(), mock_env(), info, msg)
    }
    ...
    fn _pause_relayer_api(
        ...
        let msg = ExecuteMsg::PauseRelayerApi { since_block };
        execute(deps.as_mut(), mock_env(), info, msg)
    }
    ...
    pub fn pause_relayer_api(
        ...
        _pause_relayer_api(deps, info, 0)
    }
    ...
    pub fn unpause_public_api(
        ...
        _pause_public_api(deps, info, u64::MAX)
    }
    ...
    pub fn unpause_relayer_api(
        ...
        _pause_relayer_api(deps, info, u64::MAX)
    }
    ...
    fn success_pause_apis_by_admin() {
        ...
        assert_query_paused(&mut deps, true, true);
    }
    ...
    fn failure_unpause_apis_by_monitor() {
        ...
        let info = mock_info(monitor, &coins(0, DEFAULT_DENOM));
        pause_public_api(&mut deps, info.clone()).unwrap();
        pause_relayer_api(&mut deps, info.clone()).unwrap();
...
mod swap {
    ...
    pub fn swap(deps: DepsMut, from: &str, destination: &str, amount: u128) -> StdResult<Response> {
        let msg = ExecuteMsg::Swap {
            destination: destination.to_string(),
        };

        let info = mock_info(from, &coins(amount, DEFAULT_DENOM));
...
mod reverse_swap {
    ...
    pub fn reverse_swap(
        ...
        let msg = ExecuteMsg::ReverseSwap {
            rid: rid,
            to: addr!(to),
    ...
    fn success_reverse_swap() {
        ...
        match &response.messages[0].msg {
            CosmosMsg::Bank(bank) => match bank {
                ...
                _ => panic!("unexpected bank message in handle response"),
            },
    ...
    fn failure_reverse_swap_not_relayer() {
        ...
        expect_error!(response, ERR_ACCESS_CONTROL_ONLY_RELAYER);
        assert_state_unchanged(deps.as_ref(), deposited);
    }
    ...
    fn failure_reverse_swap_supply_exceeded() {
        ...
        assert_state_unchanged(deps.as_ref(), 0u128);
    }
...
mod refund {
    ...
    pub fn refund_in_full(
        ...
        let msg = ExecuteMsg::RefundInFull {
            id: id,
            to: addr!(to),
            amount: cu128!(amount),
            relay_eon: eon,
...
mod withdraw {
    ...
    fn withdraw(deps: DepsMut, caller: &str, amount: u128, recipient: &str) -> StdResult<Response> {
...
mod withdraw_fees {
    ...
    fn _do_one_reverse_swap(
        ...
        let relayer = "relayer";
        deposit(deps, DEFAULT_SWAP_LOWER_LIMIT, DEFAULT_OWNER).unwrap();
        grant_role(deps, RELAYER_ROLE, relayer, DEFAULT_OWNER).unwrap();
        reverse_swap(
            deps.as_mut(),
            relayer,
            0u64,
...
mod set_limits {
    ...
    fn success_set_limits() {
        ...
        let min = 20u128;
        let max = 30u128;
        let response = set_limits(deps.as_mut(), DEFAULT_OWNER, fee, min, max).unwrap();
...
mod set_reverse_aggregated_allowance {
    ...
    fn set_reverse_aggregated_allowance_by_approver(
        ...
        grant_role(deps, APPROVER_ROLE, approver, DEFAULT_OWNER).unwrap();
        set_reverse_aggregated_allowance(deps, approver, amount)
    }
